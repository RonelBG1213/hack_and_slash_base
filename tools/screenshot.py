"""Renders a scene to a PNG with no window open.
    ...
    python tools/screenshot.py select out.png --class priest
...
from hack_and_slash.scenes.select import CharacterSelectScene  # noqa: E402


def build_scene(name: str, campaign, bestiary, atlas, seed: int, stage: int, hero: str):
    if name == "menu":
        return MenuScene(campaign, bestiary, atlas, seed=seed)
    if name == "select":
        roster = [c.id for c in bestiary.hero_classes]
        return CharacterSelectScene(
            campaign, bestiary, atlas, seed=seed, start_stage=stage,
            index=roster.index(hero) if hero in roster else 0,
        )
    if name == "play":
        return PlayScene(
            campaign, bestiary, atlas, seed=seed, start_stage=stage, hero_type_id=hero
        )
    raise SystemExit(f"unknown scene '{name}' -- try 'play', 'select' or 'menu'")
...
def main() -> int:
    ...
    parser.add_argument("scene", choices=("play", "select", "menu"))
    ...
    parser.add_argument(
        "--class", dest="hero", default=DEFAULT_HERO,
        help="which class to play, or to have highlighted on the select screen",
    )
    ...
    scene = build_scene(args.scene, campaign, bestiary, atlas, args.seed, stage, args.hero)
